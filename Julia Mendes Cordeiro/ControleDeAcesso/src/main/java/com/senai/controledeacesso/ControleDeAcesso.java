...
public class ControleDeAcesso {
    ...
    // Caminho para o arquivo bancoDeDados.txt, bancoDeRegistro.txt e para a pasta imagens. Elas estão dentro da pastaControleDeAcesso, que fica no C:\Users\SeuUsuario\
    private static final File arquivoBancoDeDados = new File(pastaControleDeAcesso, "bancoDeDados.txt");
    private static final File arquivoBancoDeRegistro = new File(pastaControleDeAcesso, "bancoDeRegistro.txt");
    ...
    public static String[][] matrizRegistrosDeAcesso = {{"", "", ""}}; // inicia a matriz com uma linha e três colunas com "" para que na primeira vez não apareça null na tabela de registros
    ...
    private static void criarNovoRegistroDeAcesso(String idAcessoRecebido) {
        ...
        salvarRegistroNoArquivo();
    }


    public static void salvarRegistroNoArquivo() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(arquivoBancoDeRegistro))) {
    ...
    private static void carregarRegistroDoArquivo() {
        try (BufferedReader reader = new BufferedReader(new FileReader(arquivoBancoDeRegistro))) {
            String linha;
            ...
            if (!conteudo.toString().trim().isEmpty()) {
                ...
                matrizRegistrosDeAcesso = new String[linhasDaTabela.length][matrizRegistrosDeAcesso.length];
    ...
    private static void verificarEstruturaDeDiretorios() {
        ...
        if (!arquivoBancoDeRegistro.exists()) {
            try {
                if (arquivoBancoDeRegistro.createNewFile()) {
                    System.out.println("Arquivo bancoDeRegistro.txt criado com sucesso.");
                } else {
                    System.out.println("Falha ao criar o arquivo bancoDeRegistro.txt.");
                }
            } catch (IOException e) {
                System.out.println("Erro ao criar arquivo bancoDeRegistro.txt: " + e.getMessage());
            }
        }
